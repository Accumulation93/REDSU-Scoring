...
最终 CMS 生成后、交给文件提交协调器前，必须重新检查最终字节：唯一签名、完整覆盖、摘要与签名值、证书绑定，以及要求清单时清单确实存在。任何错误均失败关闭，不保存半成品、不退回无签名文件。指定页码必须同时应用于签名域所在页，不能只用该页尺寸计算坐标却把域放在第一页。
...
`files[].certificates` 仅在本文件最终 PDF 与平台身份均验真后提供 `{fingerprint,derBase64}`。它是实际 PDF 签名使用的 X.509 公开叶证书，须同时匹配组织受控登记及未撤销的受保护密钥版本，DER 上限 64 KiB。未匹配、失败或历史未登记文件不导出其任意内嵌证书；不读取或输出私钥。小程序报告以 `.cer` 文件导出：手机优先微信原生分享，电脑优先保存到磁盘，能力不支持时尝试另一条路径；用户主动取消则立即停止。全部尝试结束后才清理本地临时副本，不得在首次失败后过早删除。另提供显式“复制证书文本”，只把同一公开 DER 编码成 PEM，指导保存为 `.cer`，不自动安装证书。

签名数学有效、证书名称相同，都不能作为平台签署依据。攻击者可以自造私钥并填写相同主体名称，但不能通过受保护密钥版本和组织登记的真实证书指纹匹配。界面只有在内容、签名、身份、平台证书均核实且凭证链通过或明确历史不足时，才显示托管方式和经办人；未确认来源时禁止泛化为“平台签署”，禁止提供外来证书信任入口。顶层 `passed` 也不得绕过逐文件必需检查。回归须覆盖冒用相同主体、自签 PDF 上传及复制真实清单后换钥重签。
...
Acrobat 的“至少一个签名有问题”是汇总提示，也可能对应“签名有效性未知 / 文档未被修改 / 签名者身份未知”，不能只看顶部文案就认定内容被篡改。必须展开签名面板核对内容完整性和证书信任；如果明确显示内容被修改或签名无效，不得把它解释为仅缺少信任。

2026-09-07 对用户提供的原文件现场检查：Adobe 明确显示文档未被修改、身份因不在可信列表而未知；平台 PDF 检查和独立 OpenSSL 对同一原始字节均通过，清单包含两条步骤凭证。未更改原文件或阅读器信任设置。此事实不代表所有历史文件或任何收件人的信任环境都已验证。

可用 `node server/scripts/checkPdfInteroperability.js <原文件路径>` 做只读复核。该脚本不输出人员信息、不修改 PDF；OpenSSL `-noverify` 只跳过证书信任链判断，仍验证内容与签名值。它不能代替组织授权、HMAC、身份快照和审批事件关联核验。第三方阅读器的警告只能由实际信任条件解决，不得通过删除签名、修改提示或隐藏失败状态制造可信。
