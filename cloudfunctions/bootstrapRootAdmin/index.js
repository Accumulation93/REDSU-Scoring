...
exports.main = async (event = {}) => {
  ...
  if (!hrRes.data.length || String(hrRes.data[0].name || '') !== name) {
    return {
      status: 'invalid_params',
      message: '请从 hr_info 中选择有效成员初始化至高权限管理员'
    };
  }

  const existingRootAdmin = await db.collection('admin_info')
    .where({
  ...
  if (existingRootAdmin.data.length) {
    return {
      status: 'forbidden',
      message: '系统中已存在至高权限管理员'
    };
  }
