...
exports.main = async () => {
  try {
    ...
    const rows = await getAllRecords(db.collection('identities')).catch(() => []);
    const identities = rows.map((item) => {
      const code = safeString(item.code || item['身份类别编码'] || item._id);
      return {
        id: safeString(item._id),
        key: code,
        code,
        name: safeString(item.name || item['身份类别名称']),
        sortOrder: toNumber(item.sortOrder == null ? item['排序顺序'] : item.sortOrder, 0),
        description: safeString(item.description || item['身份类别描述']),
    ...
    return { status: 'success', identities };
  } catch (error) {
    return {
      status: 'error',
      message: safeString(error && (error.message || error.errMsg)) || '获取身份类别列表失败'
    };
  }
};
